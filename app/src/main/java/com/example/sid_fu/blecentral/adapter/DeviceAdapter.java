...
//            holder.imgIcon.setImageBitmap(BitmapUtils.getImageFromAssetsFile(mContext,"logo/"+cList.get(position).getImagePath()+".png"));
        holder.btnDetails.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                App.getDeviceDao().updateDefult(cList.get(position).getId(), "true");
...
//                App.getInstance().speak("是否确定删除，如果删除，数据将会永久丢失，设备信息将无法找回！请慎重选择！");
        ...
        holder.btnNormal.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String contentString = "vlt_tpms_device" + "|" + cList.get(position).getLeft_FD() + "|" + cList.get(position).getRight_FD()
                        + "|" + cList.get(position).getLeft_BD() + "|" + cList.get(position).getRight_BD()
                        +"|"+cList.get(position).getDeviceName()+"|"+cList.get(position).getDeviceDescripe();
